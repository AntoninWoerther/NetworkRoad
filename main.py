...
class RoadNetworkApp:
    ...
    def create_interface(self):
        ...
        for color, label in legend:
            ...
            self.side_ax.text(
                0.23, y, label,
                color=C["white"], fontsize=7.5, va="center",
            )
            y -= 0.033
      ...
      #  )
        ...
        self.sliders["Construction"] = self.make_slider(
            ...
            "spd generation", 0.25, 4.0, self.build_speed, None,
        )
        self.sliders["Traffic"] = self.make_slider(
            [0.345, 0.065, 0.18, 0.019],
            "spd vehicles", 0.25, 8.0, self.traffic_speed, None,
        )
        ...
        self.generate_button = self.make_button(
            [0.58, 0.152, 0.085, 0.044], "GENERATE"
        )
        self.instant_button = self.make_button(
            [0.674, 0.152, 0.085, 0.044], "INSTANT"
        )
        self.random_button = self.make_button(
            [0.768, 0.152, 0.075, 0.044], "RANDOMIZE"
        )
        self.pause_button = self.make_button(
            [0.852, 0.152, 0.060, 0.044], "PAUSE"
        )
        self.replay_button = self.make_button(
            [0.921, 0.152, 0.055, 0.044], "REPLAY"
        )
        ...
        self.seed_feedback = self.fig.text(
            0.58, 0.058,
            "Change grid/roads, then press GENERATE",
            color=C["muted"], fontsize=7.2,
        )

        self.generate_button.on_clicked(self.generate_from_controls)
        self.instant_button.on_clicked(self.instant_build)
    ...
    def make_slider(self, bounds, label, minimum, maximum, initial, step):
        ...
        slider = Slider(
            slider_ax, label, minimum, maximum,
            valinit=initial, valstep=step, color=C["cyan"],
        )
    ...
    def make_button(self, bounds, label):
        ...
        button = Button(button_ax, label, color=C["panel"], hovercolor=C["panel_2"])
    ...
    def generate_network(self, seed=None, new_endpoints=True):
        ...
        self.total_build_edges = max(
            1, sum(max(0, len(route) - 1) for route in self.routes),
        )
    ...
    def generate_route(self, route_index, source, target):
        ...
        detour_profile = 0.0 if self.route_count <= 1 else route_index / (self.route_count - 1)
        ...
        def can_finish(x, y, last_direction, repeat_count):
            ...
            for verticals, _pattern_weight in vertical_patterns:
                for forward in forward_moves:
                    result = simulate_action(x, y, last_direction, repeat_count, verticals, forward)
                    if result is None:
                        continue
        ...
        while current.x != target_x:
            ...
            for verticals, pattern_weight in vertical_patterns:
                for forward in forward_moves:
                    result = simulate_action(
                        current.x, current.y, last_direction, repeat_count, verticals, forward)
                    if result is None:
                        continue
                    ...
                    reuse = sum(self.node_usage.get(node, 0) for node in nodes)
                    weight = pattern_weight * forward_weight / (1.0 + reuse * 0.50)
            ...
            selected = self.rng.choices(range(len(candidates)), weights=weights, k=1)[0]
            chosen_nodes, _chosen_directions, last_direction, repeat_count = candidates[selected]
        ...
        for previous, center, following in zip(route, route[1:], route[2:]):
            incoming = (center.x - previous.x, center.y - previous.y)
            outgoing = (following.x - center.x, following.y - center.y)
    ...
    def edge_would_cross(self, a, b):
        ...
        for segment in self.segments:
            ...
            if seg_low_x != low_x or seg_high_x != high_x:
                continue
            s_low = segment.start if segment.start.x == low_x else segment.end
            s_high = segment.end if segment.end.x == high_x else segment.start
            if (a_low.y - s_low.y) * (a_high.y - s_high.y) < 0:
                return True
    ...
    def compute_shortest_path(self):
        ...
        self.shortest_distance = distances.get(self.end_node, float("inf"))
        node = self.end_node
        while node in previous:
            parent, segment = previous[node]
            self.shortest_path_keys.add(self.segment_key(segment.start, segment.end))
            node = parent
    ...
    def draw_speed_signs(self):
        """
        Place a triangle at each node where the segment type changes.
        Green triangle pointing up  = acceleration (diagonal → straight).
        Red   triangle pointing down = braking     (straight → diagonal).
        """
        for patch in self.sign_patches:
            patch.remove()
        self.sign_patches = []
        ...
        for route in self.routes:
            for i in range(1, len(route) - 1):
                ...
                incoming_diagonal = self.is_diagonal(prev_node, curr_node)
                outgoing_diagonal = self.is_diagonal(curr_node, next_node)

                if incoming_diagonal == outgoing_diagonal:
                    continue

                cx, cy = curr_node.x, curr_node.y

                if not incoming_diagonal and outgoing_diagonal:
                    # Straight → diagonal : braking, red triangle pointing down
                    pts = np.array([
                        [cx - size, cy + size * 0.6],
                        [cx + size, cy + size * 0.6],
                        [cx,        cy - size * 0.6],
                    ...
                    # Diagonal → straight : acceleration, green triangle pointing up
                    pts = np.array([
                        [cx - size, cy - size * 0.6],
                        [cx + size, cy - size * 0.6],
                        [cx,        cy + size * 0.6],
                ...
                self.sign_patches.append(patch)
    ...
    def rebuild_scene(self):
        ...
        self.road_glow  = LineCollection([], colors=C["cyan"], linewidths=8.0,  alpha=0.07, zorder=2)
        self.road_lines = LineCollection([], colors=C["road"], linewidths=2.15, alpha=0.94, zorder=3)
        self.path_glow  = LineCollection([], colors=C["cyan"], linewidths=4.4,  alpha=0.90, zorder=4)
        ...
        self.ax.text(self.start_node.x + 0.25, self.start_node.y + 0.35, "START",
                     color=C["green"], fontsize=7.5, fontweight="bold", zorder=11)
        self.ax.text(self.end_node.x - 0.25, self.end_node.y + 0.35, "END",
    ...
    def current_build_edge(self):
        if self.build_finished or self.build_route_index >= len(self.routes):
            return None
        route = self.routes[self.build_route_index]
        if self.build_edge_index >= len(route) - 1:
            return None
        return route[self.build_edge_index], route[self.build_edge_index + 1]
    ...
    def advance_build(self):
        ...
        a, b = edge
        distance = max(np.hypot(b.x - a.x, b.y - a.y), 1e-9)
        self.build_edge_progress += (0.085 * self.build_speed) / distance

        if self.build_edge_progress >= 1.0:
            self.built_segment_keys.add(self.segment_key(a, b))
    ...
    def update_builder_color(self):
        if self.build_route_index >= len(self.routes):
            return
        route = self.routes[self.build_route_index]
        color = C["cyan"] if route[0] == self.start_node else C["purple"]
    ...
    def update_build_drawing(self):
        ...
        if active_edge is not None:
            a, b = active_edge
            current = self.interpolate_nodes(a, b, self.build_edge_progress)
            active_key = self.segment_key(a, b)

            if active_key not in self.built_segment_keys:
                full_roads.append([(a.x, a.y), current])
                if active_key in self.shortest_path_keys:
                    shortest.append([(a.x, a.y), current])

            self.builder_scatter.set_offsets(np.array([current]))
            self.builder_glow.set_offsets(np.array([current]))
        else:
            self.builder_scatter.set_offsets(np.empty((0, 2)))
            self.builder_glow.set_offsets(np.empty((0, 2)))
    ...
    def update_visible_nodes(self):
        ...
        junctions   = [(n.x, n.y) for n in visible_nodes if n.type == NodeType.Intersection]
        connections = [(n.x, n.y) for n in visible_nodes if n.type == NodeType.Connection]

        self.intersection_scatter.set_offsets(junctions   if junctions   else np.empty((0, 2)))
        self.connection_scatter.set_offsets(connections if connections else np.empty((0, 2)))

    # ------------------------------------------------------------------
    # VEHICLES
    # ------------------------------------------------------------------

    def segment_speed_limit(self, a, b):
        """
    ...
    def route_for_traffic_direction(self, route_index, direction):
        ...
        desired_start = self.start_node if direction == 1 else self.end_node
    ...
    def make_vehicles(self):
        ...
        for index in range(self.vehicle_count):
            ...
            self.vehicles.append({
                ...
                "path":         path,
                "edge_index":   0,
                "progress":     (index / max(self.vehicle_count, 1)) * 0.85,
    ...
    def update_vehicles(self):
        ...
        for vehicle in self.vehicles:
            ...
            if edge_index < len(path) - 1:
                a = path[edge_index]
                b = path[edge_index + 1]
                vehicle["target_speed"] = self.segment_speed_limit(a, b)

            vehicle["speed"] += (vehicle["target_speed"] - vehicle["speed"]) * SPEED_LERP
            remaining = vehicle["speed"] * self.traffic_speed
            ...
            while remaining > 0:
                ...
                edge_remaining = (1.0 - vehicle["progress"]) * length
                ...
                if remaining < edge_remaining:
                    ...
                    next_a = path[vehicle["edge_index"]]
                    next_b = path[vehicle["edge_index"] + 1]
                    vehicle["target_speed"] = self.segment_speed_limit(next_a, next_b)

            path       = vehicle["path"]
            edge_index = min(vehicle["edge_index"], len(path) - 2)
            point      = self.interpolate_nodes(
                path[edge_index], path[edge_index + 1], vehicle["progress"])
        ...
        self.vehicle_out.set_offsets(outbound       if outbound else np.empty((0, 2)))
        self.vehicle_out_glow.set_offsets(outbound  if outbound else np.empty((0, 2)))
        self.vehicle_back.set_offsets(inbound        if inbound  else np.empty((0, 2)))
        self.vehicle_back_glow.set_offsets(inbound   if inbound  else np.empty((0, 2)))
    ...
    def update_status_text(self):
        junctions = sum(node.type == NodeType.Intersection for node in self.nodes.values())
        ...
        if self.paused:
            ...
            percent   = min(100, int(100 * self.completed_build_edges / max(self.total_build_edges, 1)))
            direction = "→" if self.build_route_index % 2 == 0 else "←"
            state     = f"BUILD {self.build_route_index + 1}/{self.route_count} {direction} {percent:02d}%"
        ...
        self.stat_labels["shortest"].set_text(
            f"{self.shortest_distance:.1f}" if np.isfinite(self.shortest_distance) else "--")
        self.stat_labels["longest"].set_text(
            f"{max(self.route_lengths):.1f}" if self.route_lengths else "--")
    ...
    def instant_build(self, _event=None):
        """Marque tous les segments comme construits et passe en mode trafic."""
        for segment in self.segments:
            self.built_segment_keys.add(segment.key)
        self.build_finished        = True
    ...
    def generate_from_controls(self, _event=None):
        ...
        self.generate_network(seed=self.seed, new_endpoints=True)
        self.rebuild_scene()
        self.seed_feedback.set_text("Generated with current seed")
    ...
    def new_routes_same_endpoints(self, _event=None):
        dimensions_changed = (self.pending_cols != self.cols or self.pending_rows != self.rows)
        self.pending_route_count = int(self.sliders["Routes"].val)
        self.route_count         = self.pending_route_count
        self.seed                = random.SystemRandom().randint(0, MAX_SEED)
        self.paused              = False
        ...
        if dimensions_changed:
            self.apply_pending_configuration()
            new_endpoints = True
            message       = "Grid changed · new endpoints generated"
        else:
            new_endpoints = False
            message       = "New alternating roads · endpoints preserved"

        self.generate_network(seed=self.seed, new_endpoints=new_endpoints)
        self.rebuild_scene()
        self.sync_seed_box()
